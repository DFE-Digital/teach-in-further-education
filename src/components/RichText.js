...
const options = {
  renderMark: {
    [MARKS.BOLD]: text => (
      <span className="govuk-!-font-weight-bold">{text}</span>
    ),
  ...
  renderNode: {
    ...
    [BLOCKS.HEADING_2]: (node, children) => (
      <h2 className="govuk-heading-l">{children}</h2>
    ),
    ...
    [BLOCKS.OL_LIST]: (node, children) => (
      <ul className="govuk-list govuk-list--number govuk-list--spaced">
        {children}
      </ul>
    ),
...
export const RichText = ({ data, className }) => {
  return (
    <div className={"app-rich-text " + className}>
      {documentToReactComponents(JSON.parse(data), options)}
    </div>
  )
}
